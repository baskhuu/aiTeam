...
## 変更影響ファイルの確認ルール（厳守）
新機能の追加・仕様変更・スクリプト変更を行った際は、コミット前に以下を確認すること:

影響を受ける可能性があるファイル:
- `README.md` — セットアップ手順・ファイル構成・使い方の記述
- `CLAUDE.md` — ルール・コマンド仕様・フローの記述
- `.github/copilot-instructions.md` — Gemini/Copilot向け規約
- `.ai/AITEAM_HISTORY.md` — 作業サマリー・TODOセクション

確認の観点:
- 変更した機能・コマンド・ファイルパスがドキュメントに記載されているか
- 記載されている場合、内容が変更後の実態と一致しているか
- 古い手順・廃止された仕様が残っていないか
- **修正対象と同じ内容を記載している他ファイルも必ず確認すること**（同一内容の重複記載は複数ファイルに存在しうる）

過去の実例:
- npm パッケージ化実装後、README.md の旧セットアップ手順が残存
- `copilot-instructions.md` のコミット形式を修正したが、元凶の `CLAUDE.md` 本体が未修正のまま残存
